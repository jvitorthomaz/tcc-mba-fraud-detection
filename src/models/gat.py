import torch
import torch.nn.functional as F

from torch_geometric.nn import GATConv

from sklearn.metrics import (
    f1_score,
    precision_score,
    recall_score,
    average_precision_score
)

from src.models.gcn import build_pyg_data, evaluate
from src.evaluation.plots import plot_confusion_matrix, plot_pr_curve

import pandas as pd
import os
from collections import Counter

from src.utils.config import GCN_LR, GCN_EPOCHS


class GAT(torch.nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.conv1 = GATConv(in_channels, 64, heads=4)
        self.conv2 = GATConv(64 * 4, 32, heads=1)
        self.lin = torch.nn.Linear(32, 2)

    def forward(self, data):
        x, edge_index = data.x, data.edge_index

        x = F.elu(self.conv1(x, edge_index))
        x = F.elu(self.conv2(x, edge_index))
        x = self.lin(x)

        return x


def train(model, data, train_mask, optimizer, weights):
    model.train()
    optimizer.zero_grad()

    out = model(data)

    loss = F.cross_entropy(
        out[train_mask],
        data.y[train_mask],
        weight=weights
    )

    loss.backward()
    optimizer.step()

    return loss.item()


def run_gat(df_train, df_val, df_test, edges):

    print("\n===== GAT MODEL =====")

    df_all = pd.concat([df_train, df_val, df_test])
    data = build_pyg_data(df_all, edges)

    n_train = len(df_train)
    n_val = len(df_val)

    train_mask = torch.zeros(data.num_nodes, dtype=torch.bool)
    val_mask = torch.zeros(data.num_nodes, dtype=torch.bool)
    test_mask = torch.zeros(data.num_nodes, dtype=torch.bool)

    train_mask[:n_train] = True
    val_mask[n_train:n_train+n_val] = True
    test_mask[n_train+n_val:] = True

    class_counts = Counter(df_train["class"].values)
    total = sum(class_counts.values())

    weights = torch.tensor([
        total / class_counts[0],
        total / class_counts[1]
    ], dtype=torch.float)

    model = GAT(in_channels=data.num_features)
    optimizer = torch.optim.Adam(model.parameters(), lr=GCN_LR)

    for epoch in range(1, GCN_EPOCHS + 1):
        loss = train(model, data, train_mask, optimizer, weights)

        if epoch % 10 == 0:
            y_true_val, _, y_prob_val = evaluate(model, data, val_mask)
            val_pr_auc = average_precision_score(y_true_val, y_prob_val)

            print(f"Epoch {epoch} | Loss {loss:.4f} | Val PR-AUC {val_pr_auc:.4f}")

    y_true, y_pred, y_prob = evaluate(model, data, test_mask)

    results = {
        "PR_AUC": average_precision_score(y_true, y_prob),
        "F1": f1_score(y_true, y_pred, zero_division=0),
        "Precision": precision_score(y_true, y_pred, zero_division=0),
        "Recall": recall_score(y_true, y_pred),
        "model": "GAT"
    }

    print("\n===== RESULTADOS GAT =====")
    print(results)

    os.makedirs("results/figures", exist_ok=True)
    plot_confusion_matrix(y_true, y_pred, "GAT")
    plot_pr_curve(y_true, y_prob, "GAT")

    os.makedirs("results/new_tables", exist_ok=True)
    pd.DataFrame([results]).to_csv("results/new_tables/gat_results.csv", index=False)

    return results


# import torch
# import torch.nn.functional as F

# from torch_geometric.nn import GATConv
# from torch_geometric.data import Data

# from sklearn.metrics import (
#     f1_score,
#     precision_score,
#     recall_score,
#     average_precision_score
# )

# from src.evaluation.plots import (
#     plot_confusion_matrix,
#     plot_pr_curve
# )

# import pandas as pd
# import os

# from src.utils.config import GCN_LR, GCN_EPOCHS


# # ==============================
# # 1. CONSTRUIR DATASET
# # ==============================

# def build_pyg_data(df, edges):
#     df = df.reset_index(drop=True)

#     id_map = {tx: i for i, tx in enumerate(df["txId"].values)}

#     X = df.drop(columns=["txId", "class"]).values
#     X = torch.tensor(X, dtype=torch.float)

#     y = torch.tensor(df["class"].values, dtype=torch.long)

#     edge_index = edges.copy()
#     edge_index = edge_index[
#         edge_index["txId1"].isin(id_map) & edge_index["txId2"].isin(id_map)
#     ]

#     edge_index = edge_index.replace({"txId1": id_map, "txId2": id_map})
#     edge_index = torch.tensor(edge_index.values.T, dtype=torch.long)

#     return Data(x=X, edge_index=edge_index, y=y)


# # ==============================
# # 2. MODELO GAT
# # ==============================

# class GAT(torch.nn.Module):
#     def __init__(self, in_channels):
#         super().__init__()

#         self.conv1 = GATConv(in_channels, 64, heads=4, concat=True)
#         self.conv2 = GATConv(64 * 4, 32, heads=1, concat=True)

#         self.lin = torch.nn.Linear(32, 2)

#     def forward(self, data):
#         x, edge_index = data.x, data.edge_index

#         x = self.conv1(x, edge_index)
#         x = F.elu(x)

#         x = self.conv2(x, edge_index)
#         x = F.elu(x)

#         x = self.lin(x)

#         return x


# # ==============================
# # 3. TREINAMENTO
# # ==============================

# def train(model, data, train_mask, optimizer):
#     model.train()
#     optimizer.zero_grad()

#     out = model(data)

#     loss = F.cross_entropy(out[train_mask], data.y[train_mask])
#     loss.backward()
#     optimizer.step()

#     return loss.item()


# # ==============================
# # 4. AVALIAÇÃO
# # ==============================

# def evaluate(model, data, mask):
#     model.eval()

#     with torch.no_grad():
#         logits = model(data)

#         probs = torch.softmax(logits, dim=1)[:, 1].cpu().numpy()
#         preds = logits.argmax(dim=1).cpu().numpy()
#         y_true = data.y.cpu().numpy()

#     mask = mask.cpu().numpy()

#     y_true = y_true[mask]
#     preds = preds[mask]
#     probs = probs[mask]

#     return y_true, preds, probs


# # ==============================
# # 5. PIPELINE GAT
# # ==============================

# def run_gat(df_train, df_val, df_test, edges):

#     print("\n===== GAT MODEL =====")

#     df_all = pd.concat([df_train, df_val, df_test])

#     data = build_pyg_data(df_all, edges)

#     n_train = len(df_train)
#     n_val = len(df_val)

#     train_mask = torch.zeros(data.num_nodes, dtype=torch.bool)
#     val_mask = torch.zeros(data.num_nodes, dtype=torch.bool)
#     test_mask = torch.zeros(data.num_nodes, dtype=torch.bool)

#     train_mask[:n_train] = True
#     val_mask[n_train:n_train+n_val] = True
#     test_mask[n_train+n_val:] = True

#     model = GAT(in_channels=data.num_features)
#     optimizer = torch.optim.Adam(model.parameters(), lr=GCN_LR)

#     # ==============================
#     # TREINO
#     # ==============================

#     for epoch in range(1, GCN_EPOCHS + 1):
#         loss = train(model, data, train_mask, optimizer)

#         if epoch % 10 == 0:
#             y_true_val, _, y_prob_val = evaluate(model, data, val_mask)
#             val_pr_auc = average_precision_score(y_true_val, y_prob_val)

#             print(f"Epoch {epoch} | Loss {loss:.4f} | Val PR-AUC {val_pr_auc:.4f}")

#     # ==============================
#     # TESTE FINAL
#     # ==============================

#     y_true, y_pred, y_prob = evaluate(model, data, test_mask)

#     test_metrics = {
#         "PR_AUC": average_precision_score(y_true, y_prob),
#         "F1": f1_score(y_true, y_pred, zero_division=0),
#         "Precision": precision_score(y_true, y_pred, zero_division=0),
#         "Recall": recall_score(y_true, y_pred),
#         "model": "GAT"
#     }

#     print("\n===== RESULTADOS GAT =====")
#     print(test_metrics)

#     # ==============================
#     # PLOTS
#     # ==============================

#     os.makedirs("results/figures", exist_ok=True)

#     plot_confusion_matrix(y_true, y_pred, "GAT")
#     plot_pr_curve(y_true, y_prob, "GAT")

#     # ==============================
#     # SALVAR CSV
#     # ==============================

#     os.makedirs("results/tables", exist_ok=True)

#     results_df = pd.DataFrame([test_metrics])
#     results_df.to_csv("results/tables/gat_results.csv", index=False)

#     print("\nResultados salvos em: results/tables/gat_results.csv")

#     return results_df
